...
# Function to log batch test info
batch_log() {
...
check_system_requirements() {
    log "Checking system requirements for batch size testing..."
    ...
    if [ "$available_mem" -lt 16384 ]; then
        warn "Low memory available. Some batch sizes may not be optimal."
    fi
    ...
    if [ "$available_disk" -lt 30 ]; then
        error "Insufficient disk space. Need at least 30GB for testing."
        exit 1
    fi
...
# Function to create configuration for specific batch size
create_batch_config() {
    local batch_size="$1"
    local test_dir="$2"
    local config_file="$test_dir/config_batch_${batch_size}.toml"
    
    log "Creating configuration for batch size: $batch_size"
...
buffer_size = 524288
chunk_size = 16777216
max_inflight_bytes = $((524288 * batch_size * 2))
max_retries = 5
retry_delay_ms = 2000
...
# Function to start system monitoring for batch test
start_batch_monitoring() {
    local test_name="$1"
    local batch_size="$2"
    local monitor_file="$RESULTS_DIR/${test_name}_batch_${batch_size}_monitoring.csv"
    
    log "Starting system monitoring for batch size: $batch_size"
    
    # Create monitoring header
    echo "timestamp,batch_size,cpu_usage,memory_used_mb,memory_available_mb,disk_io_read_mb,disk_io_write_mb,network_rx_mb,network_tx_mb,active_processes,transfer_progress,success_rate" > "$monitor_file"
    ...
    (
        while true; do
            ...
            # Simulate transfer progress and success rate
            local transfer_progress=$((RANDOM % 100))
            local success_rate=$((85 + RANDOM % 15))  # 85-100%
            
            echo "$timestamp,$batch_size,$cpu_usage,$memory_used,$memory_available,$disk_read,$disk_write,$network_rx,$network_tx,$active_processes,$transfer_progress,$success_rate" >> "$monitor_file"
    ...
    monitor_pid=$!
    echo "$monitor_pid" > "$RESULTS_DIR/${test_name}_batch_${batch_size}_monitor.pid"
    
    log "Monitoring started for batch size $batch_size (PID: $monitor_pid)"
}

# Function to stop batch monitoring
stop_batch_monitoring() {
    local test_name="$1"
    local batch_size="$2"
    local pid_file="$RESULTS_DIR/${test_name}_batch_${batch_size}_monitor.pid"
    
    if [ -f "$pid_file" ]; then
        local monitor_pid=$(cat "$pid_file")
        if kill -0 "$monitor_pid" 2>/dev/null; then
            log "Stopping monitoring for batch size $batch_size (PID: $monitor_pid)"
            kill "$monitor_pid"
...
run_batch_size_test() {
    ...
    create_batch_config "$batch_size" "$test_dir"
    create_batch_transfer_config "$batch_size" "$test_dir"
    
    # Start monitoring
    start_batch_monitoring "$TEST_NAME" "$batch_size"
    
    # Create test files
    log "Creating $batch_size test files..."
    for i in $(seq 1 $batch_size); do
        local size=$((10 + RANDOM % 20))  # 10-30MB files
    ...
    wait
    log "Created $batch_size test files"
    ...
    (
        ...
        for i in $(seq 1 $batch_size); do
            echo "Processing transfer $i/$batch_size..."
        ...
        # Determine success
        if [ $completed -ge $((batch_size * 90 / 100)) ]; then
    ...
    log "Batch size $batch_size test completed in ${duration}s"
    
    # Stop monitoring
    stop_batch_monitoring "$TEST_NAME" "$batch_size"
    
    # Generate batch results summary
    local batch_summary="$test_dir/batch_${batch_size}_summary.txt"
    {
        echo "=== BATCH SIZE $batch_size TEST SUMMARY ==="
        echo "Test: $TEST_NAME"
        echo "Batch size: $batch_size"
        ...
        grep "Total transfers:" "$execution_log" | tail -1
        grep "Successfully completed:" "$execution_log" | tail -1
        grep "Failed:" "$execution_log" | tail -1
        grep "Retried:" "$execution_log" | tail -1
        grep "Estimated throughput:" "$execution_log" | tail -1
        echo
        echo "Files:"
        echo "  Execution Log: $execution_log"
        echo "  Configuration: $test_dir/config_batch_${batch_size}.toml"
        echo "  Batch Config: $test_dir/batch_${batch_size}_transfers.json"
        echo "  Monitoring Data: $RESULTS_DIR/${TEST_NAME}_batch_${batch_size}_monitoring.csv"
        
    } > "$batch_summary"
    
    log "Batch summary saved to: $batch_summary"
    echo
}
...
trap 'log "Cleaning up..."; pkill -f "batch.*monitoring" 2>/dev/null || true' EXIT
