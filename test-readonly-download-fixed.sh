...
# Load config - export variables directly
...
export SFTP_REMOTE_PATH=$(grep "^SFTP_REMOTE_PATH=" .env | cut -d= -f2)
...
# Setup SFTP command - use sshpass if available, otherwise prompt
if command -v sshpass >/dev/null 2>&1; then
    ...
    # Use SSH_ASKPASS or expect
    SFTP_CMD="sftp -o StrictHostKeyChecking=no -P $SFTP_PORT $SFTP_USER@$SFTP_HOST"
...
if echo "ls" | eval "$SFTP_CMD" >/dev/null 2>&1; then
